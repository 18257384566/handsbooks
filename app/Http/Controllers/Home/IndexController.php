...
class IndexController extends Controller
{
    public function show()
    {
        ...
        $top_list = array();
        foreach($res as $k => $v){
            $top_list[] = Book::select('books.*','auths.name')->join('auths','books.au_id','auths.id')->find($v->books_id);
        }
        ...
        foreach($result as $k => $v){
            ...
            $book[$v->au_id] = Book::select('title','id')->where('au_id',$v->au_id)->limit(3)->get();
        }
        ...
        /*广告*/
        $ad = Ad::where('status',1)->orderBy('created_at','desc')->limit(3)->get();

        return view('home.index',compact('top_list','new','auth','book','ad'))->with('result',$result);
    }
}
